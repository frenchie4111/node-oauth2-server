...
function checkClient (done) {
  var self = this;
  this.model.getClient.bind(this)(this.client.clientId, this.client.clientSecret,
      function (err, client) {
...
function useAuthCodeGrant (done) {
  ...
  var self = this;
  this.model.getAuthCode.bind(this)(code, function (err, authCode) {
    if (err) return done(error('server_error', false, err));
...
function usePasswordGrant (done) {
  ...
  var self = this;
  return this.model.getUser.bind(this)(uname, pword, function (err, user) {
    if (err) return done(error('server_error', false, err));
    if (!user) {
...
function useRefreshTokenGrant (done) {
  ...
  var self = this;
  this.model.getRefreshToken.bind(this)(token, function (err, refreshToken) {
    if (err) return done(error('server_error', false, err));
    ...
    if (self.model.revokeRefreshToken) {
      return self.model.revokeRefreshToken.bind(this)(token, function (err) {
        if (err) return done(error('server_error', false, err));
        done();
      });
    }
...
function useClientCredentialsGrant (done) {
  ...
  var self = this;
  return this.model.getUserFromClient.bind(this)(clientId, clientSecret,
      function (err, user) {
    if (err) return done(error('server_error', false, err));
    if (!user) {
...
function useExtendedGrant (done) {
  var self = this;
  this.model.extendedGrant.bind(this)(this.grantType, this.req,
...
function checkGrantTypeAllowed (done) {
  this.model.grantTypeAllowed.bind(this)(this.client.clientId, this.grantType,
...
function saveAccessToken (done) {
  ...
  this.model.saveAccessToken.bind(this)(accessToken, this.client.clientId, expires,
...
function saveRefreshToken (done) {
  ...
  this.model.saveRefreshToken.bind(this)(refreshToken, this.client.clientId, expires,
      this.user, function (err) {
    if (err) return done(error('server_error', false, err));
    done();
    });
...
function postGrant(done) {
  this.model.postGrant.bind(this)(function(err) {
    if (err) return done(error('server_error', false, err));
    done();
  });
}
...
function sendResponse (done) {
  ...
  if (this.user) response.user_id = this.user.id;
